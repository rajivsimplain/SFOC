...
/****** Object:  StoredProcedure [dbo].[GoldETL_IncrementalLoad]    Script Date: 2/5/2025 11:11:23 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[GoldETL_IncrementalLoad]
@RUNID INT
...
BEGIN
		...
		--SET @RUNID=(SELECT IFRRUNID FROM IFRUNSTATUS WHERE IFRIFID=900);
	 ...
	 --  SELECT * INTO #ETL_SITDGENE FROM ETL_SITDGENE WHERE RUNID=@RUNID;
	   ...
	   SELECT * INTO #ETL_ARTULUL FROM ETL_ARTULUL WHERE RUNID=@RUNID;
	...
	INSERT INTO ETL_ARTRAC
	SELECT 0 AS Runid,Processid,StatusNo,StatusDesc,'N',CreateDate,CreateBy,ModifiedDate,ModifiedBy,ARTCINR,ARTCEXR,ARTTYPP,ARTNATU,ARTETAT,ARTCENT,ARTGEST,ARTCFAB,ARTACOM,ARTCOLL,ARTSTAT,ARTQUAL,
     ...
     DELETE A FROM ETL_ARTATTRI A WHERE EXISTS (SELECT 1 FROM  #ETL_ARTATTRI  B WHERE A.AATCINR =B.AATCINR AND A.AATCCLA=B.AATCCLA AND A.AATDDEB=B.AATDDEB) AND A.RUNID=0 ; 
		...
		INSERT INTO ETL_ARTATTRI
		SELECT 0 as RUNID,Processid,StatusNo,StatusDesc,OP_Flag,CreateDate,CreateBy,ModifiedDate,ModifiedBy,AATCINR,AATCCLA,AATCATT,AATVALN,AATDDEB,AATDFIN,AATDCRE,AATUTIL,AATNMOD,AATDMAJ,AATVALNUM,AATVALDAT,
		...
		INSERT INTO ETL_ARTUC
		SELECT 0 AS RUNID,Processid,StatusNo,StatusDesc,OP_Flag,CreateDate,CreateBy,ModifiedDate,ModifiedBy,ARACINR,ARASEQVL,ARACEXTA,ARACFIN,ARACCIN,ARANFILF,ARADDEB,ARADFIN,ARARESID,ARASITE,ARACINL,ARAREFC,
		ARACEXR,ARACEXVL,ARATFOU,ARAORIG,ARATCDE,ARADCRE,ARADMAJ,ARAUTIL,ARACDBLE,ARACEAN,ARACALC,ARAAGRS,ARAIPRX,ARAMUA,ARAMINCDE,ARATPSR,ARAMAXCDE,ARACTLR,ARAMOTCOM,ARARET,ARALIBACH
         ...
         DELETE A FROM ETL_ARTUV A WHERE EXISTS (SELECT 1 FROM  #ETL_ARTUV B WHERE A.ARVCINV=B.ARVCINV ) AND A.RUNID=0 ; 

		 INSERT INTO ETL_ARTUV
		 SELECT 0 AS RUNID,Processid,StatusNo,StatusDesc,OP_Flag,CreateDate,CreateBy,ModifiedDate,ModifiedBy,
	 	 ARVCINR,ARVCINV,ARVCLIBL,ARVCLIBC,ARVCEXR,ARVCEXV,ARVETAT,ARVPPRIX,ARVUUVC,ARVPUVC,ARVPERM,ARVNIEL,ARVOBLG,ARVCHBA,ARVGRAT,ARVGESP,ARVPOD,
			ARVGEST,ARVDCRE,ARVDMAJ,ARVUTIL,ARVCAFL,ARVTINS,ARVRFAB,ARVGUEL,ARVTOPS,ARVDOBS,ARVPVNEG,ARVGESTDOM,ARVNSERIE,ARVCONGEST,
			ARVDECPCT,ARVDECPDS,ARVUMES,ARVLONG,ARVLARG,ARVHAUT,ARVMULTVA,ARVDATEFV,ARVTYPUV,ARVPNET,ARVUPDS,ARVDVCONSO,ARVDACTIV,
			ARVPCTMGEXTS,ARVPCTMGPTOT,ARVTYPMESPX,ARVUTINS,ARVDDEL,ARVRETPHY,ARVRETFIN,ARVRETDAYS,ARVINVRESERV FROM #ETL_ARTUV ;
		 ...
		 DELETE A FROM ETL_ARTCOCA A WHERE EXISTS (SELECT 1 FROM  #ETL_ARTCOCA  B WHERE A.ARCCINR =B.ARCCINR AND A.ARCCODE=B.ARCCODE AND A.ARCDDEB=B.ARCDDEB) AND A.RUNID=0; 
		 	INSERT INTO ETL_ARTCOCA
			SELECT 0 AS RUNID,Processid,StatusNo,StatusDesc,OP_Flag,CreateDate,CreateBy,ModifiedDate,ModifiedBy,ARCCINV,ARCCINR,ARCTCOD,ARCCODE,ARCETAT,ARCDDEB,ARCDFIN,ARCIETI,ARCDCRE,ARCDMAJ,ARCUTIL FROM #ETL_ARTCOCA B;-- WHERE B.RUNID=@RUNID AND B.OP_FLAG IN ('U','I');
		 ...
		 DELETE A FROM ETL_STRUCOBJ A WHERE EXISTS (SELECT 1 FROM  #ETL_STRUCOBJ  B WHERE A.SOBCINT =B.SOBCINT) AND A.RUNID=0; 
			...
			SELECT 0 as Runid,Processid,StatusNo,StatusDesc,OP_Flag,CreateDate,CreateBy,ModifiedDate,ModifiedBy,SOBIDSTR,SOBIDNIV,SOBCEXT,SOBCINT,SOBCEXTIN,SOBDCRE,SOBDMAJ,SOBUTIL,SOBNMOD,SOBDDLC,SOBNLOT,SOBSNID FROM #ETL_STRUCOBJ B ;--WHERE B.RUNID=@RUNID AND B.OP_FLAG IN ('U','I');
		 ...
		 DELETE A FROM ETL_STRUCREL A WHERE EXISTS (SELECT 1 FROM  #ETL_STRUCREL  B WHERE A.OBJCINT =B.OBJCINT AND A.OBJPERE=B.OBJPERE AND A.OBJDDEB=B.OBJDDEB) AND A.RUNID=0; 
		  ...
		  SELECT 0 AS RUNID,Processid,StatusNo,StatusDesc,OP_Flag,CreateDate,CreateBy,ModifiedDate,ModifiedBy,OBJCINT,OBJPERE,OBJPRINT,OBJDDEB,OBJDFIN,OBJDCRE,OBJDMAJ,OBJUTIL,OBJNMOD FROM #ETL_STRUCREL B;-- WHERE B.RUNID=@RUNID AND B.OP_FLAG IN ('U','I');
 ...
 DELETE A FROM ETL_AVESCOPE A WHERE EXISTS ( SELECT 1 FROM #ETL_AVESCOPE B WHERE 
	...
	[PATDCRE] ,	
	[PATDMAJ] ,	
	[PATUTIL]  FROM #ETL_OPRATTRI;


	 DELETE A FROM ETL_SITATTRI A WHERE EXISTS (SELECT 1 FROM #ETL_SITATTRI B WHERE 
	 A.[SATATT] =B.SATATT AND A.[SATCLA] =B.SATCLA AND 	A.[SATDDEB]=B.SATDDEB AND A.[SATSITE]=B.SATSITE) AND RUNID=0;

	INSERT INTO ETL_SITATTRI 
	SELECT 0 Runid,Processid,StatusNo,StatusDesc,OP_Flag,CreateDate,CreateBy,ModifiedDate,ModifiedBy,
	[SATSITE],
	[SATCLA],
	[SATATT] ,
	...
	[SATVALNUM],
	[SATVALDAT] ,
	[SATVALHRE] FROM #ETL_SITATTRI B ;

	

--	SELECT * FROM ETL_Table_COLUMNS WHERE TABLE_ID=19
	
	
	 DELETE A FROM ETL_ARTULUL A WHERE EXISTS (SELECT 1 FROM #ETL_ARTULUL B WHERE  A.ALLCINLF=B.ALLCINLF AND A.ALLCINLP=B.ALLCINLP) AND RUNID=0;

	INSERT INTO ETL_ARTULUL
	SELECT 0 Runid,Processid,StatusNo,StatusDesc,OP_Flag,CreateDate,CreateBy,ModifiedDate,ModifiedBy,
	ALLCINLP,
ALLCINLF,
ALLTYPL,
...
ALLUTIL,
ALLSEQVLP,
ALLSEQVLF FROM #ETL_ARTULUL;


/*
	 DELETE A FROM ETL_RESEAU A WHERE EXISTS (SELECT 1 FROM #ETL_RESEAU B WHERE  A.ALLCINLF=B.ALLCINLF AND A.ALLCINLP=B.ALLCINLP) AND RUNID=0;
