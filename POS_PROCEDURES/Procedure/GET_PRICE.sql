...
/****** Object:  UserDefinedFunction [dbo].[GET_PRICE]    Script Date: 2/5/2025 11:27:14 PM ******/
SET ANSI_NULLS ON
GO
...
/*SELECT @P_Date as EFFDT,storenum AS STORENUM,ITEM,UPC,CONVERT(DATE,EFFFROM) EFFFROM,CONVERT(DATE,EFFTHRU) EFFTHRU,PRCTYPE,CONVERT(INT,PRCMULTI) PRCMULTI,PRICE,EVENTCD
...
(
    SELECT distinct	 
		...
		aviddeb as PEFFFROM,
		avidfin as PEFFTHRU,
		avimulti as PAVImulti,
		OPLCEXOPR AS EVENTCODE,
    ...
    FROM 
		...
		JOIN ETL_ARTUV  V ON ARVCINV=AVICINV AND ARVETAT=1 AND V.RUNID=0
	   ...
	   WHERE 
		...
		AND RESSITE in (SELECT GoldStore FROM GoldPos_Store_map) 
