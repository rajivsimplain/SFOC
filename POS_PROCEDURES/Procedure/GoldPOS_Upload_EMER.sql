...
/****** Object:  StoredProcedure [dbo].[GoldPOS_Upload_EMER]    Script Date: 2/5/2025 11:12:12 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[GoldPOS_Upload_EMER]
@RUNID INT,
@PROCESSID INT
...
--	select 1 GoldStore,'0093' active_posstore  into #teststore
  ...
  END;
	...
	set @RECCNT=1;
	   ...
	   SELECT * INTO #ETL_AVEPRIX FROM ETL_AVEPRIX WHERE RUNID=@RUNID AND AVIMALIG=90; --EMERGENCY PRICE CHANGE
	   ...
	   SELECT AATCINR AS ITEM ,'U' AS ITEM_FLAG FROM #ETL_ARTATTRI WHERE UPPER(AATVALN) IN ('E','EMERGENCY') 
	     	   )  T
...
(
   ...
   --      CASE WHEN (RESDFIN=CONVERT(DATE,GETDATE()) OR CONVERT(DATE,GETDATE()) = AVODFIN) THEN
	...
	--	ELSE
		...
		OPLCEXOPR,
		PATATT,
		CASE WHEN @usemulti='Y' AND  PATATT NOT LIKE 'REG%' THEN 1 else isnull(I.AVIMULTI,1) end AVIMULTI,
		--AVINTAR,
		--AVOPRIO,
    ...
    FROM 
		...
		JOIN #ETL_AVEPRIX B ON ARVCINV=B.AVICINV --AND O.AVORESCINT=B.AVORESCINT --AND I.AVINTAR=B.AVINTAR		--EMERGENCY PRICE CHANGE
  ...
  --AND NOT EXISTS (select 1 from ETL_OPRATTRI z where z.PATATT=A.PATATT and Z.PATNOPR=(SELECT OPLNOPR FROM ETL_OPRPLAN B WHERE OPLCEXOPR='DISC'))    --promo discontinue
      ...
      WHERE 
		...
		AND CONVERT(DATE,GETDATE()) BETWEEN B.aviddeb AND B.avidfin  
...
(
    ...
    WHERE 
       CONVERT(DATE,GETDATE()) BETWEEN aveddeb AND avedfin
		...
		AND CONVERT(DATE,GETDATE())  BETWEEN ARCDDEB AND ARCDFIN	
		AND CONVERT(DATE,GETDATE())  BETWEEN ARADDEB AND ARADFIN
		AND ((CONVERT(DATE,GETDATE()) BETWEEN RESDDEB AND RESDFIN) ) --STORE END	
		AND CONVERT(DATE,GETDATE()) BETWEEN Q.ARADDEB AND Q.ARADFIN 
		...
		/*
		     SELECT AATCINR,AATCCLA,AATCATT,AATVALN ,AATVALNUM,AATDDEB,AATDFIN INTO #TAB
				FROM ETL_ARTATTRI A,#ALLITEMS B
						WHERE CAST(GETDATE() AS DATE) BETWEEN A.AATDDEB AND A.AATDFIN 
		                 AND A.RUNID=0 AND A.AATCINR=B.ITEM	;
...
RAISERROR('Get Coupons....',10,1) WITH NOWAIT
  ...
  -- select * from #ALLITEMS
	 ---**Bottle Deposit *****
	 RAISERROR('Get Bottle....',10,1) WITH NOWAIT
...
RAISERROR('Get Bottle1....',10,1) WITH NOWAIT
...
--	DROP TABLE #BDEPOSIT
	...
	--drop if exist allitems;

	/*select * into allitems from #ALLITEMS
   ...
   --select LnkdItmupc into UNUSEDCPN from LINKED_ITM_DTL where not exists(select 1 from pos_store_audit where large_linked_to='000000'+(convert(varchar(10),LnkdItmupc)))				 
	...
	--RAISERROR('Insert STG_POS_ITEM tables',10,1) WITH NOWAIT
							...
							SELECT REQUEST_ID ,GETDATE(),NULL,
				               NULL,'UPDATE','','staged',(select distinct PosStore from GoldPos_Store_map)  FROM STG_POS_ITEMS WHERE RUNID=@RUNID AND EXISTS (SELECT 1 FROM POS_UPDATE_ITEM WHERE batch_id=@RUNID)
...
/*
	   SELECT LnkdItmupc INTO #UNUSEDCPN from LINKED_ITM_DTL a ,STG_POS_ITEMS B where RUNID=@RUNID AND a.LnkdItmupc<>ISNULL(LLINKEDUPC,0)
	   AND NOT EXISTS (SELECT 1 FROM pos_store_audit v 
	   WHERE substring(v.large_linked_to,7,len(large_linked_to))=LnkdItmupc AND V.UPC<>B.UPC )*/
	  ...
	  RAISERROR('Coupon Delete...',10,1) WITH NOWAIT
...
set @request_id =(SELECT ISNULL((SELECT MAX(request_id)+1 from pos_item_job_sweeper),1));
 ...
 set @request_id =(SELECT ISNULL((SELECT MAX(request_id)+1 from pos_item_job_sweeper),1));
	...
	INSERT INTO POS_DEPOSIT_ITEM (id,created_at,department,deposit_type,name,price,pricing_method,quantity,request_id,status,upc)
				...
				--and not exists(SELECT 1 FROM POS_STORE_AUDIT X WHERE X.UPC=A.UPC AND X.sale_price=a.PRICE)
