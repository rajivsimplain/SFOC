...
/****** Object:  StoredProcedure [dbo].[GoldPOS_Upload_reviewfix]    Script Date: 2/5/2025 11:12:54 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[GoldPOS_Upload_reviewfix]
@RUNID INT,
@PROCESSID INT
...
--	IF ISNULL(@RECCNT,0)>0 
	...
	--	OR  ((A.AATVALN<>B.AATVALN) AND A.AATCCLA='X1DESC')
		...
		OR ISNULL(A.AATCATT,0)<>ISNULL(B.AATCATT,0) AND (A.AATCCLA LIKE ('SGN%') OR A.AATCCLA LIKE('LBL%')))
		AND  CAST(GETDATE() AS DATE) BETWEEN A.AATDDEB AND A.AATDFIN
		...
		---ARTUC FOR LABEL PROCESS ONLY --vendor and vendor prod num change
	...
	--Price Tables
		...
		UPDATE A SET OP_FLAG='UP' FROM ETL_AVEPRIX A , ETL_AVEPRIX B WHERE A.AVICINV=B.AVICINV 
		...
		--ENDDATE CHANGE
		UPDATE A SET OP_FLAG='UP' FROM ETL_AVETAR A,ETL_AVETAR B WHERE A.AVENTAR=B.AVENTAR AND B.RUNID=0
		...
		UPDATE A SET OP_FLAG='UP' FROM ETL_AVESCOPE A WHERE  ---NOT EXISTS (SELECT 1 FROM ETL_AVESCOPE B WHERE A.AVONTAR=B.AVONTAR AND B.RUNID=0 )
	   ...
	   SELECT*  INTO #ETL_RESEAU  FROM ETL_RESEAU WHERE RUNID=@RUNID;
        ...
        UPDATE ETL_AVEPRIX SET OP_FLAG='P' WHERE RUNID=0 AND (CONVERT(DATE,GETDATE())=AVIDDEB OR CONVERT(DATE,GETDATE())=AVIDFIN) ;

		UPDATE ETL_RESEAU SET OP_FLAG='P' WHERE RUNID=0 AND (CONVERT(DATE,GETDATE())=RESDDEB OR CONVERT(DATE,GETDATE())=RESDFIN) ;

		UPDATE ETL_AVESCOPE SET OP_FLAG='P' WHERE RUNID=0 AND (CONVERT(DATE,GETDATE())=AVODDEB OR CONVERT(DATE,GETDATE())=AVODFIN) ;
		
		UPDATE ETL_AVETAR SET OP_FLAG='P' WHERE RUNID=0 AND (CONVERT(DATE,GETDATE())=AVEDDEB OR CONVERT(DATE,GETDATE())=AVEDFIN) ;

		UPDATE ETL_OPRATTRI SET OP_FLAG='P' WHERE RUNID=0 AND (CONVERT(DATE,GETDATE())=PATDDEB OR CONVERT(DATE,GETDATE())=PATDFIN) ;
...
--	DROP TABLE #BDEPOSIT
	...
	INSERT INTO STG_POS_ITEMS
	      ...
	      --  (SELECT right('000'+ substring(AATCCLA,6,3),3) FROM #DEPT WHERE  AATCINR=ITEM)  DEPT_CODE,  --C1
			(SELECT distinct right('000'+ ( [dbo].[get_posdept](RESSITE,SUBDEPT)),3) FROM #DEPT WHERE  AATCINR=ITEM)  DEPT_CODE,
		...
		null as points,
 		...
 		--	( SELECT DISTINCT TSOBDESC FROM ETL_TRA_STRUCOBJ WHERE TSOBCINT=ARVCLIBL AND RUNID=0 AND LANGUE='US' ) AS LBLDESC,	--	PRINT @LBL 
			...
			(SELECT  distinct CASE RIGHT( [dbo].[get_posdept](RESSITE,SUBDEPT),2) WHEN '01' THEN 'G' WHEN '02' THEN 'P' WHEN '03' THEN 'M' 
   ...
   --select LnkdItmupc into UNUSEDCPN from LINKED_ITM_DTL where not exists(select 1 from pos_store_audit where large_linked_to='000000'+(convert(varchar(10),LnkdItmupc)))				 
	...
	RAISERROR('Insert STG_POS_ITEM tables',10,1) WITH NOWAIT
							...
							SELECT REQUEST_ID ,GETDATE(),NULL,
				               NULL,'UPDATE','','test',(select distinct PosStore from GoldPos_Store_map)  FROM STG_POS_ITEMS WHERE RUNID=@RUNID AND EXISTS (SELECT 1 FROM POS_UPDATE_ITEM WHERE batch_id=@RUNID)
...
set @request_id =(SELECT ISNULL((SELECT MAX(request_id)+1 from pos_item_job_sweeper),1));
		...
		INSERT INTO pos_coupon_item (id,created_at,price_type,request_id,sequence,status,type,upc,value,name,department,promotion_code)
			SELECT NEXT VALUE FOR SEQ_CPNID,
				GETDATE(),3,@request_id,1,'test',CASE WHEN Lnkditmtyp=7 then 'store' END,UPC,LnkdItmVal--((REGPRICE*t.ItemsPerCpn)-LnkdItmVal) --need to confrim is it right
      	...
      	INSERT INTO pos_coupon_item (id,created_at,price_type,request_id,sequence,status,type,upc,value,name,department)
			SELECT NEXT VALUE FOR SEQ_CPNID,
				GETDATE(),3,@request_id,1,'test',CASE WHEN PRODTYP=7 then 'store' END,UPC,PRICE,ITEM_NAME,DEPT_CODE 
		...
		IF @@ROWCOUNT>0 OR @ROWCNT>0
           INSERT INTO pos_item_job_sweeper values (@request_id,GETDATE(),null,null,'COUPON',NULL,'DELTA LOAD ','test',@posstore);
 ...
 set @request_id =(SELECT ISNULL((SELECT MAX(request_id)+1 from pos_item_job_sweeper),1));  
	...
	INSERT INTO POS_DEPOSIT_ITEM (id,created_at,department,deposit_type,name,price,pricing_method,quantity,request_id,status,upc)
				 ...
				 dname,price,0,1,@request_id,'test',upc  FROM
			...
			IF @@ROWCOUNT>0 
           INSERT INTO pos_item_job_sweeper values (@request_id,GETDATE(),null,null,'DEPOSIT',NULL,'DELTA LOAD ','test',@posstore);
  ...
  set @request_id =(SELECT ISNULL((SELECT MAX(request_id)+1 from pos_item_job_sweeper),1));   
	...
	SALE_QUANTITY,TAXABLEA,UPC,VALUE_CARD_TYPE,WIC_ALLOWED,ERROR_MESSAGE,REQUEST_ID,STATUS)
	   SELECT 
				  ...
				  1,TAX,UPC,NULL,WIC,NULL,@request_id,'test'  --,case when LINKEDCODE IS NOT NULL THEN 1 ELSE 0 END   
	  FROM STG_POS_ITEMS A 
	...
	SALE_QUANTITY,TAXABLEA,UPC,VALUE_CARD_TYPE,WIC_ALLOWED,ERROR_MESSAGE,REQUEST_ID,STATUS,ldq_Deal_quantity,ldq_Limit_quantity)
	   SELECT 
				 ...
				 -- CASE WHEN LLINKEDUPC IS NOT NULL THEN REGPRICE ELSE isnull(PRICE,0) END ,
				  CASE WHEN (LLINKEDUPC IS NULL OR LLINKEDUPC=0) THEN isnull(PRICE,0) ELSE REGPRICE END,
				  1,TAX,UPC,NULL,WIC,NULL,@request_id,'test',1,99
	  FROM STG_POS_ITEMS A 
	  WHERE OP_FLAG='I' and  RUNID=@runid and UPC IS NOT NULL  and STORE=@gstore AND PRODTYP=0 AND ISNULL(GPRODTYPE,'A')='ZERO';

	 	IF @@ROWCOUNT>0 OR @ROWCNT>0
	  INSERT INTO pos_item_job_sweeper values (@request_id,GETDATE(),null,null,'NEW',NULL,'DELTA LOAD ','test',@posstore);
  ...
  set @request_id =(SELECT ISNULL((SELECT MAX(request_id)+1 from pos_item_job_sweeper),1));
	...
	SALE_QUANTITY,TAXABLEA,UPC,VALUE_CARD_TYPE,WIC_ALLOWED,ERROR_MESSAGE,REQUEST_ID,STATUS,tax_planf)
	   SELECT 
				...
				--  CASE WHEN LLINKEDUPC IS NOT NULL THEN REGPRICE ELSE isnull(PRICE,0) END ,
				 CASE WHEN (LLINKEDUPC IS NULL OR LLINKEDUPC=0) THEN isnull(PRICE,0) ELSE REGPRICE END,
				  1,TAX,UPC,NULL,WIC,NULL,@request_id,'test',case when LINKEDCODE IS NOT NULL THEN 1 ELSE 0 END   --LINK TO DEP FLAG
	  FROM STG_POS_ITEMS A 
	...
	SALE_QUANTITY,TAXABLEA,UPC,VALUE_CARD_TYPE,WIC_ALLOWED,ERROR_MESSAGE,REQUEST_ID,STATUS,ldq_Deal_quantity,ldq_Limit_quantity)
	  ...
	  WHERE OP_FLAG='U' and RUNID=@runid and UPC IS NOT NULL  and STORE=@gstore AND PRODTYP=0 AND ISNULL(GPRODTYPE,'A')='ZERO';
	 	...
	 	IF @@ROWCOUNT>0 OR @ROWCNT>0
	  INSERT INTO pos_item_job_sweeper values (@request_id,GETDATE(),null,null,'UPDATE',NULL,'DELTA LOAD ','test',@posstore);
	  ...
	  INSERT INTO POS_DELETE_ITEM  SELECT  NEXT VALUE FOR SEQ_POSID,NULL,GETDATE(),'NULL',@request_id,'test',UPC FROM STG_POS_ITEMS 
	  WHERE OP_FLAG='D' and RUNID=@runid and UPC IS NOT NULL  and STORE=@gstore ;

	   INSERT INTO POS_DELETE_ITEM  SELECT  NEXT VALUE FOR SEQ_POSID,NULL,GETDATE(),'NULL',@request_id,'test',UNCPN FROM #UNUSEDCPN

	  	IF @@ROWCOUNT>0 
	  INSERT INTO pos_item_job_sweeper values (@request_id,GETDATE(),null,null,'DELETE',NULL,'DELTA LOAD ','test',@posstore);  
